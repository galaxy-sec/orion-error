...
错误处理策略定义了系统在面对不同类型错误时的应对机制。合理的错误处理策略能够提高系统的可靠性、可用性和用户体验。本系统提供四种核心错误处理策略，每种策略都有其特定的适用场景和实现方式。

## 策略矩阵

### 策略对比总览

| 策略 | 适用场景 | 实现方式 | 示例 | 可恢复性 | 复杂度 |
|------|----------|----------|------|----------|--------|
| **向上抛出** | 当前层无法处理的错误 | `ErrConv` trait 转换错误类型 | 存储错误转业务错误 | 取决于上层处理 | 低 |
| **重试** | 临时性故障 | `retry` 宏包裹操作 | 数据库连接失败自动重连 | 高 | 中 |
| **降级** | 非关键路径失败 | 返回默认值或简化功能 | 推荐服务失败返回空列表 | 部分 | 高 |
| **熔断** | 持续错误避免系统崩溃 | 断路器模式 | 连续支付失败停止交易 | 自动恢复 | 高 |
